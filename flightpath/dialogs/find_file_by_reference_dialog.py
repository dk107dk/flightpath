...
class FindFileByReferenceDialog(QDialog):
    def __init__(self, *, main):
        ...
        self.named_x_name.setSizeAdjustPolicy(
        ...
        self.box = IconPackager.add_svg_icon(
            main=main, widget=self.name_one, on_click=on_click, icon_path=paths
        )
    ...
    def _show_context_menu(self, position) -> None:
        ...
        if index.isValid():
            ...
            if self.datatype.currentText() == "results":
                if self.name_one.text().endswith(
                    ...
                    open_file_action.triggered.connect(
                        lambda: self.reference_file_handler._open_results_file(row)
                    )
                    ...
                    #
                    open_file_action.triggered.connect(
                        lambda: self.reference_file_handler._open_origin_file(row)
                    )
                    open_file_action.setText("Open source data file")

                    show_run_action = QAction()
                    show_run_action.triggered.connect(
                        lambda: self.reference_file_handler._show_run_dir(row)
                    )
                    show_run_action.setText("Show run")
            else:
                open_file_action.setText("Open data file")
                open_file_action.triggered.connect(
                    lambda: self.reference_file_handler._open_files_file(row)
                )

            show_metadata_action = QAction()
            show_metadata_action.setText("Show metadata")
            show_metadata_action.triggered.connect(
                lambda: self.reference_file_handler._show_manifest(row)
            )
            #
            #
            #
            ...
            copy_path_action.triggered.connect(
                lambda: self.reference_file_handler._copy_path(row)
            )
